 ...
 * 尾部优先——错误信息通常在末尾。
 */
export async function truncateOutput(content: string, maxLines = MAX_OUTPUT_LINES): Promise<string> {
...
const runBash: AgentTool = {
  name: 'run_bash',
  description: '执行 shell 命令。参数：command（命令字符串）、timeout（可选，秒，默认 30，最大 300——扫描类命令如 nmap 需加大）。返回 stdout+stderr，截取最后 200 行。',
  parameters: {
    type: 'object',
    properties: {
  ...
  execute: async (args, signal) => {
    ...
    const timeoutMs = Math.min(300, Math.max(1, timeout ?? 30)) * 1000
    try {
      const { stdout, stderr } = await execAsync(command, { maxBuffer: 1024 * 1024, timeout: timeoutMs, signal })
