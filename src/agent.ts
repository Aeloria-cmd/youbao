...
import { stream, buildAssistantMessage, buildToolResultMessage, type Model, type Context, type Usage } from './llm.js'
...
export type AgentEvent =
  ...
  | { type: 'turn_end'; stopReason: 'end_turn' | 'max_tokens' | 'aborted' | 'error'; message?: string }

/** compaction 触发阈值：消息数超过此值时压缩旧消息。
 *  V4 模型 1M 上下文 + 输入缓存 1/10 价，上下文不再稀缺——
 *  阈值放宽以保留更多细节(compaction 越少,渗透推理丢的信息越少) */
...
async function compactContext(model: Model, context: Context, signal?: AbortSignal): Promise<void> {
  ...
  // 切点保护：recent 不能以孤儿 tool_result 开头（缺了配对的 tool_call，API 会 400）。
  // 切点落在 tool_result 消息上就往前挪，把它连同 assistant 轮一起留给摘要。
  let cut = Math.max(0, context.messages.length - KEEP_RECENT)
  const isToolResultMsg = (m: Context['messages'][number]) =>
    typeof m.content !== 'string' && m.content.some(b => b.type === 'tool_result')
  while (cut > 0 && isToolResultMsg(context.messages[cut])) cut--

  const oldMessages = context.messages.slice(0, cut)
  const recentMessages = context.messages.slice(cut)
  // 把旧消息序列化成纯文本让 LLM 总结(剥离 reasoning——思维链对摘要是噪音且烧 token)
  const conversationText = oldMessages
    .map(m => {
      if (typeof m.content === 'string') return `${m.role}: ${m.content}`
      const stripped = m.content.filter(b => b.type !== 'reasoning')
      return `${m.role}: ${JSON.stringify(stripped)}`
    })
    .join('\n')
...
export async function* runAgent(
  ...
  while (true) {
    ...
    // 1. 流式调用 LLM，收集文本、reasoning(思考模式推理,需回传)和 tool_calls
    let text = ''
    let reasoning = ''
    ...
    for await (const ev of stream(model, context, { tools: toolDefs, signal })) {
      if (ev.type === 'text_delta') {
        ...
        if (ev.stopReason === 'aborted') {
          ...
          context.messages.push(buildAssistantMessage(text, [], reasoning))
        ...
        yield { type: 'turn_end', stopReason: 'error', message: ev.error.message }
        return
      }
    }

    // 2. 把 assistant 回复塞回 context
    context.messages.push(buildAssistantMessage(text, toolCalls, reasoning))
